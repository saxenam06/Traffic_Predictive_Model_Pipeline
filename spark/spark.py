...
def main():
    ...
    trafficdata=sqlContext.read.csv("./streaming-pipelines/data", header="True")
    ...
    stationdata=sqlContext.read.csv("./streaming-pipelines/data_station", header="True")
    ...
    stationTraffic=stationTraffic.select(stationTraffic.id,\
    	...
    	stationTraffic.latitude.cast("double"), \
	stationTraffic.longitude.cast("double").alias("longitudeW"), \
    	stationTraffic.lane_of_travel.alias("lanes").cast("int"), \
	stationTraffic.functional_classification_name.alias("type"), \
